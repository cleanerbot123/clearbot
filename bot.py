...
async def nuke(ctx, count: int = 10):
    """Create channels named nuke-1, nuke-2, etc. Usage: !nuke [count]"""
    if count < 1 or count > 50:
        await ctx.send("❌ Count must be between 1 and 50.")
        return

    await ctx.send(f"💣 Creating {count} nuke channels...")
    created = 0
    for i in range(1, count + 1):
        try:
            await ctx.guild.create_text_channel(f"nuke-{i}")
            created += 1
            await asyncio.sleep(0.5)
        except discord.HTTPException as e:
            await ctx.send(f"❌ Stopped at {created}: {e}")
            break

    await ctx.send(f"✅ Created {created} nuke channel(s)!")


@bot.command(name="nukeall")
@commands.has_permissions(administrator=True)
async def nukeall(ctx):
    """Delete all nuke-* channels. Usage: !nukeall"""
    nuke_channels = [c for c in ctx.guild.text_channels if c.name.startswith("nuke-")]

    if not nuke_channels:
        await ctx.send("❌ No nuke channels found.")
        return

    await ctx.send(f"🧹 Deleting {len(nuke_channels)} nuke channel(s)...")
    deleted = 0
    ...
    await ctx.send(f"✅ Deleted {deleted} nuke channel(s).")
